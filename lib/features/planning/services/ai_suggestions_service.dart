...
- **CONTINUITÉ GÉOGRAPHIQUE (priorité absolue, vérifiée à l'insert)** : pour CHAQUE journée, TOUTES tes propositions doivent tenir dans **un même quartier ou secteur** (≤ 2 km à vol d'oiseau entre la 1ère et la dernière activité de la journée). Le voyageur ne doit JAMAIS faire plus de 15 min de trajet entre deux activités consécutives sur une même journée. Exemple concret pour Nancy : si la matinée est sur la Place Stanislas, le déjeuner doit être à ≤500m (pas à Jarville-la-Malgrange ni Vandœuvre). Si tu sens qu'un lieu pertinent est trop éloigné, REPLANIFIE-le sur un autre jour où il fera partie d'un cluster géographique cohérent. Mauvais : matin centre-ville + dîner en banlieue. Bon : matin + midi + après-midi + soir tous dans le même quartier (centre, Vieille Ville, port, etc.). Les adresses des activités existantes sont listées entre [📍 ...] — utilise-les pour ancrer chaque jour autour d'un secteur.
- 🚫 **PAS DE BÂTIMENTS PUREMENT ADMINISTRATIFS** : ne propose JAMAIS comme activité de visite : mairies sans intérêt patrimonial, préfectures, tribunaux, banques, supermarchés, bureaux de poste, centres administratifs, agences immobilières/voyages, parkings, stations-services, commissariats. EXCEPTION : si le bâtiment EST un monument touristique reconnu (ex: "Hôtel de Ville de Paris" classé Renaissance, "Hôtel de Ville de Bruxelles" gothique brabançon), tu peux le proposer SEULEMENT en activité Culture/Visite et préciser la valeur patrimoniale dans le titre ("Hôtel de Ville de Paris (façade Renaissance)").
