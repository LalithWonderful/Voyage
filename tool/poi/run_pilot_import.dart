...
void main(List<String> args) async {
  if (args.isEmpty) {
    stderr.writeln(
      'Usage: dart run tool/poi/run_pilot_import.dart <fixture.json> [--write]',
    );
    stderr.writeln('');
    stderr.writeln(
      'For real writes, provide secret key (preferred) or service role key:',
    );
    stderr.writeln(
      '  dart run --define=SUPABASE_URL=<url> --define=SUPABASE_SECRET_KEY=<key> ...',
    );
    stderr.writeln(
      '  dart run --define=SUPABASE_URL=<url> --define=SUPABASE_SERVICE_ROLE_KEY=<key> ...',
    );
    exit(1);
  }
  ...
  if (!dryRun) {
    try {
      assertLiveSupabaseAllowedForPoiTool(
        operation: poiPilotImportWriteOperation,
      );
  ...
  if (!dryRun && (supabaseUrl.isEmpty || writeKey.isEmpty)) {
    stderr.writeln(
      'ERROR: SUPABASE_URL and a write key (SUPABASE_SECRET_KEY or SUPABASE_SERVICE_ROLE_KEY) must be set for real writes.',
    );
    stderr.writeln(
      'Run with: dart run --define=SUPABASE_URL=<url> --define=SUPABASE_SECRET_KEY=<key> ...',
    );
    exit(1);
  }
